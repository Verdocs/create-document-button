...
export class CreateEnvelopeService implements OnDestroy {
  ...
  open<D = any, R = any>(config?: FixedDialogConfig<D>, mode?: 'dashboard' | 'embed' | 'liveview' | 'signerview' | 'builder' | 'preview'): FixedDialogRef<R> {
    ...
    if (mode) {
      dialogRef._containerInstance.mode = mode;
    }
