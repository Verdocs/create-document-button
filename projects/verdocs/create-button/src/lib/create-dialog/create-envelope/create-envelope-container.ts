...
import { EmailValidator, TemplatesGuardService, TemplatesService, TemplateSenderTypes, DocumentsService, PhoneValidator, getRGB, getRGBA } from '@verdocs/sdk';
import { PlansDialog, Broadcast } from '@verdocs/essentials';
import { LiveViewService } from '@verdocs/sdk';
import { find, orderBy } from 'lodash';
import { Subscription } from 'rxjs';

import { Overlay } from '../overlay';
import { FixedDialogConfig } from './fixed-dialog-config';
import { EnvelopeCreatedDialogComponent } from '../envelope-created-dialog/envelope-created-dialog.component';
...
export class CreateEnvelopeContainer implements OnInit, OnDestroy {
  ...
  public rSecureUrl_frontend_url: string;
  public rAccount_frontend_url: string;
  public rForm_frontend_url: string;
  ...
  _templateSource: 'dashboard' | 'embed' | 'liveview' | 'signerview' | 'builder' | 'preview';
  ...
  constructor(
    ...
    private envelopeService: DocumentsService,
  ...
  editRecipient(readOnly, role_name, role_color) {
    if (!readOnly) {
      ...
      const focusTimer = setTimeout(() => {
        const fullnameInput = this._document.getElementById('roleFullnameInput');
        fullnameInput.focus()
  ...
  verifyWithEnter(event) {
    if (event.key === 'Enter') {
      this.verifyAndMinimizeRecipientDialog();
    }
  }
      ...
      if (this.roleFormGroup.status === 'VALID') {
        ...
        if (this._templateSource === 'liveview') {
          this.liveViewService.createLiveViewEnvelope(body, this.templateToken)
            ...
            .catch((err) => {
              if (err.error.code === 'E000020') {
                const plansDialog = this.dialog.open(PlansDialog, {
                  panelClass: 'confirmation'
                });
                plansDialog.componentInstance.type = 'free';
                const user_id = this.currentProfile.user_id.substring(6);
                plansDialog.componentInstance.redirectUrl = `${this.rAccount_frontend_url}/rAccount/user/${user_id}/billing`;
              } else {
                const plansDialog = this.dialog.open(PlansDialog, {
                  panelClass: 'confirmation'
                });
                plansDialog.componentInstance.dialogTitle = 'Failed to send a Verdoc';
                plansDialog.componentInstance.dialogMessage = 'A network has occurred while sending the document. Please try again.';
              }
              console.error({
  ...
  actionAfterSend(envelope, sign?) {
    if (this.mode === 'dashboard') {
      this._broadcast.broadcast('dashboard-refresh', { refresh: true });
    }
    if (sign) {
  ...
  canSendEnvelope() {
    if (this._templateSource === 'liveview') {
      this.canSend = true
    } else if (typeof (this.canSend) !== 'boolean' && this.templateService) {
      this.canSend = this.templateService.canSendEnvelope(this.template);
    }
    return this.canSend;
  }
  ...
  signIn() {
    if (window) {
      window.location.href = `${this.rSecureUrl_frontend_url}/login?redirect_url=${window.location.href}`;
    }
  }

  signUp() {
    if (window) {
      window.location.href = `${this.rSecureUrl_frontend_url}/signup?redirect_url=${window.location.href}`;
    }
  }
  ...
  private goToEnvelopeView(envelope) {
    ...
    if (window && !!this.currentProfile && this.currentProfile.email === recipient.email) {
      window.location.href = window.location.origin +
        `/view/sign/${envelope.id}/roleName/${recipient.role_name}/invitation/${encodeURIComponent(recipient.in_app_access_key)}`;
    }
  }
